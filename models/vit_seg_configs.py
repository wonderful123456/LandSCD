...
def get_b8_config():
    ...
    # 修改 patch 大小为 8x8
    config.patches = ml_collections.ConfigDict({'size': (8, 8)})  # ViT-B/8 使用 8x8 patches
    config.hidden_size = 768
    config.transformer = ml_collections.ConfigDict()
    config.transformer.mlp_dim = 3072
    config.transformer.num_heads = 12
    ...
    # 修改预训练路径为 ViT-B_8.npz
    config.pretrained_path = '../model/vit_checkpoint/imagenet21k/ViT-B_8.npz'

    # patch_size 对应 ViT-B/8
    config.patch_size = 8  # 对应 ViT-B/8 使用的 patch 大小
