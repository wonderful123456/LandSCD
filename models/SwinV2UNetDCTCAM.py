...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.multiDctcam = ChannelMultiAttentionBlock(embed_dim * 2 ** 3, 2)
    ...
    def forward_up_features(self, x, x_downsample):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = self.multiDctcam(x.reshape(-1, x.shape[2], H, W), H, W).reshape(-1, H*W, x.shape[2]) + x
                x = layer_up(x)
            else:
    ...
    def forward(self, x):
        ...
        x, features = self.forward_up_features(x, x_downsample)
        x = self.up_x4(x)
        return x, features
...
# -------------------------------------------Network----------------------------------------------
class SwinUNetDCTCAM(nn.Module):
    def __init__(self, img_size=256,
                 ...
                 window_size=8,
                 mlp_ratio=4., ):
        super(SwinUNetDCTCAM, self).__init__()
...
if __name__ == '__main__':
    model = SwinUNetDCTCAM().to('cuda')
    imgA = torch.randn(8, 3, 256, 256).to('cuda')
    imgB = torch.randn(8, 3, 256, 256).to('cuda')
    # print(model(imgA, imgB)[2].shape)
    print(model(imgA, imgB)[1].shape)