...
class SCanNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=6, input_size=256):
    ...
    def forward(self, x1, x2):
        ...
        return F.upsample(out1, x_size[2:], mode='bilinear'), F.upsample(out2, x_size[2:], mode='bilinear'), F.upsample(change, x_size[2:], mode='bilinear')

if __name__ == '__main__':
    img_A = torch.randn(1, 3, 256, 256).to('cuda')
    img_B = torch.randn(1, 3, 256, 256).to('cuda')
    device = torch.device("cuda")
    img = torch.randn(4, 6, 256, 256)
    model = SCanNet().to(device)
