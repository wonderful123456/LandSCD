...
from loss.loss import DiverseExpertLoss
...
class CDTrainer():
    def __init__(self, args, dataloaders):  # 初始化参数
        ...
        #                              momentum=0.9,
        #                              weight_decay=5e-4)
        self.optimizer_G = optim.Adam([{'params': self.net_G.to(self.device).parameters()},
                                       {'params': self.aggregation_weight, 'lr': self.lr * 0.1}],
        ...
        if args.loss == 'ce':
            if self.num_expert == 1:
                ...
                self._pxl_loss = DiverseExpertLoss()  # SegmentationLosses()
                self._pxl_loss_CH = SegmentationLosses()
            # self._pxl_loss = SegmentationLosses()
    ...
    def _forward_pass(self, batch, is_val=False):
        self.batch = batch
        ...
        if is_val:
            self.aggregation_softmax = torch.nn.functional.softmax(self.aggregation_weight)
            _, _, self.G_pred_CH, self.outsA, self.outsB = self.net_G(img, img_B)
            expert1_logits_outputA = self.outsA[:,0,:]
            expert2_logits_outputA = self.outsA[:,1,:]
            expert3_logits_outputA = self.outsA[:,2,:]
            expert1_logits_outputB = self.outsB[:,0,:]
            expert2_logits_outputB = self.outsB[:,1,:]
            expert3_logits_outputB = self.outsB[:,2,:]
            self.G_pred = self.aggregation_softmax[0] * expert1_logits_outputA + self.aggregation_softmax[1] * expert2_logits_outputA + \
                self.aggregation_softmax[2] * expert3_logits_outputA
            self.G_pred_B = self.aggregation_softmax[0] * expert1_logits_outputB + self.aggregation_softmax[1] * expert2_logits_outputB + \
                self.aggregation_softmax[2] * expert3_logits_outputB
        else:
            self.G_pred, self.G_pred_B, self.G_pred_CH, self.outsA, self.outsB = self.net_G(img,
                                                                                            img_B)  # self.net_G(img, img_B)[0], self.net_G(img, img_B)[1], self.net_G(img, img_B)[2]
    ...
    def _backward_G(self):
        ...
        if self.num_expert == 1:
            ...
            self.G_loss = self._pxl_loss(output_logits=self.G_pred,
                                         target=gt.squeeze(dim=1),
                                         extra_info=self.outsA)
            self.G_loss_B = self._pxl_loss(output_logits=self.G_pred_B,
                                           target=gt_B.squeeze(dim=1),
                                           extra_info=self.outsB)

        # Change
        gt_CH = self.batch[4].to(self.device).long()
        self.G_loss_CH = self._pxl_loss_CH.FocalLoss(self.G_pred_CH, gt_CH.squeeze(dim=1))
        self.G_loss_total = self.G_loss_CH + self.G_loss + self.G_loss_B
        self.G_loss_total.backward()

        # 这里添加梯度裁剪
        torch.nn.utils.clip_grad_norm_(self.net_G.parameters(), max_norm=2.0)
        torch.nn.utils.clip_grad_norm_([self.aggregation_weight], max_norm=2.0)
...
if __name__ == '__main__':
    img = torch.randn(2, 6, 256, 256).argmax(dim=1)
    label = torch.ones(2, 256, 256)
    loss = DiceLoss()
    print(loss(img, label))